...
APP_SRC=./scripts/_app_scaffolds/mac/myapp.app
...
for APP_ARCH in "${APP_ARCH_LIST[@]}"; do
    ...
    if [ -e "./scripts/preproc-mac.sh" ]; then
        echo "  Running pre-processor ..."
        . ./scripts/preproc-mac.sh
    fi
    ...
    if [ -e "./scripts/postproc-mac.sh" ]; then
        echo "  Running post-processor ..."
        . ./scripts/postproc-mac.sh
    fi
